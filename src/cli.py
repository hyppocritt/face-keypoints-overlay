...
def get_parser(main: bool = False) -> ArgumentParser:
    ...
    parser.add_argument("--data", type=str, default=None, help="Path to images.")
    parser.add_argument(
        "--config", type=str, default=None, help="Path to YAML config file."
    )
    parser.add_argument(
        "--model", type=str, default=None, help="Path to model state dictionary."
    )
    parser.add_argument(
        "--metadata",
        type=str,
    ...
    parser.add_argument(
        "--mask", type=str, default=None, help="Type of mask for overlay."
    )
...
def cli_to_settings(
    cli_args: Namespace = None, cli_overrides: list[str] = None
) -> Settings:
    """
    ...
    config_path = (
        Path(cli_args.config).resolve() if (cli_args.config is not None) else None
    )
