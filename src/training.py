...
from src.utils.metrics_torch import mae_torch, mse_torch, nme_torch
            ...
            with torch.no_grad(), torch.amp.autocast("cuda", enabled=use_amp):
                ...
                if val_loss >= best_val_loss - early_stopping_delta:
                    ...
                    if current_patience == 0:
                        print("No val MSE decrease, stopping training")
...
def get_train_val_loaders(
    ...
    train_metadata, val_metadata = train_test_split(
        metadata, test_size=0.1, random_state=78
    )
    ...
    train_dataset = FacePointsTransformDataset(
        image_dir=dataset_dir, metadata_df=train_metadata, **dataset_params
    )
...
def main(settings: Settings):
    ...
    if settings.model.model_type.lower() == "resnet":
        ...
        raise ValueError(
            f'Unknown model type: {settings.model.model_type}. Use "base" or "resnet".'
        )
